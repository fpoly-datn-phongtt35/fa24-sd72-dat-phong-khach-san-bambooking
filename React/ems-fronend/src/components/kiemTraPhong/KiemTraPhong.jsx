...
import { getListRoomByCondition } from "../../services/KiemTraPhongService";
import {
  Box,
  Button,
  Container,
...
const KiemTraPhong = () => {
      ...
      try {
        ...
        if (!response.data || response.data.length === 0) {
          Swal.fire({
            icon: "info",
            title: "Thông báo",
            text: "Không có phòng nào cần kiểm tra. Vui lòng kiểm tra lại sau.",
        ...
        Swal.fire({
          icon: "error",
          title: "Lỗi",
          text: "Không thể tải danh sách phòng cần kiểm tra.",
          confirmButtonText: "Đóng",
        });
        setLoading(false);
      }
  ...
  const handleCheckRoom = (idXepPhong) => {
    Swal.fire({
      ...
      text: "Bạn có muốn bắt đầu kiểm tra phòng này?",
  ...
  const handleViewDetails = (row) => {
    ...
    Swal.fire({
      icon: "info",
      title: "Xem chi tiết",
      text: `Đang hiển thị chi tiết cho phòng ${row.tenPhong}`,
      timer: 1500,
