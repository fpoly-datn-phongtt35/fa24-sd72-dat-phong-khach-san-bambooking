...
const ThanhToanModal = ({ show, onClose, thanhToan, setHoaDon }) => {
  ...
  const [tienThanhToan, setTienThanhToan] = useState(
  ...
  const [noiDungThanhToan, setNoiDungThanhToan] = useState(
    `Thanh toan hoa don ${thanhToan?.hoaDon?.id || ""}`
  );
  ...
  useEffect(() => {
    ...
    if (thanhToan?.hoaDon?.id) {
      ...
      Swal.fire({
        icon: "error",
        title: "Lỗi",
        text: "Không có thông tin hóa đơn để thanh toán!",
        confirmButtonText: "Đóng",
      });
    }
  }, [thanhToan, setHoaDon]);
            ...
            >
              <Stack spacing={2} alignItems="center">
                ...
                <img
                  ...
                  )}&amount=${hoaDon.tongTien}&addInfo=${encodeURIComponent(
                    noiDungThanhToan
                  )}`}
                  style={{
                    width: "210px",
