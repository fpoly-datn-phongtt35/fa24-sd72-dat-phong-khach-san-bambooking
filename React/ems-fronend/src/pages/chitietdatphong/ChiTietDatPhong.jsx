...
const ChiTietDatPhong = () => {
  ...
  const handleHuyTTDP = (ttdp) => {
    ...
    if (confirmCancel) {
      Promise.all(
        ttdp.originalTTDPs.map((item) =>
          huyTTDP(item.maThongTinDatPhong)
        )
      )
        .then(() => {
    ...
    setSelectedTTDPs((prevSelected) => {
      ...
      if (isSelected) {
        return prevSelected.filter(
          (item) => !allOriginalTTDPs.includes(item)
        );
      } else {
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            {Array.isArray(grTTDP) && grTTDP.length > 0 ? (
              grTTDP.map((ttdp) => (
                ...
                >
                  ...
                  <TableCell>
                    {phongData[ttdp.maThongTinDatPhong]?.phong?.tenLoaiPhong || (
                      <Chip
                        size="small"
      ...
      <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 3, gap: 2 }}>
        ...
        <Button
          ...
          onClick={() => selectedTTDPs.forEach((ttdp) => handleCheckin({ originalTTDPs: [ttdp] }))}
          disabled={
            selectedTTDPs.length === 0 ||
            isDangDatPhong ||
