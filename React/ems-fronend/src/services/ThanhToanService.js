...
export const createThanhToan = (data) => {
    return axios.post(`${apiThanhToan}`, data, {
        headers: {
...
export const updateThanhToan = async (id, data) => {
    try {
        const response = await axios.put(`${apiThanhToan}?id=${id}`, data, {
            headers: {
                'Content-Type': 'application/json',
            },
        });
...
export const changeStatusInvoice = (id) => {
    return axios.put(`${apiHoaDon}/${id}/status`); //Front-end không chấp nhận method PATCH nếu chưa được cấu hình
};
