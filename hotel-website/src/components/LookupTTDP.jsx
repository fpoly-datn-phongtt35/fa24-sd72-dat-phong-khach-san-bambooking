...
import { getTTDPByidDatPhongTC } from "../services/TTDP.js";
import {
  getDichVuSuDungTC,
  getHoaDonByIdTC,
...
export default function LookupTTDP() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const bookingResponse = await getTTDPByidDatPhongTC(idDatPhong);
      ...
      for (const hoaDon of hoaDonList) {
        ...
        const [
          ...
          getThongTinHoaDonByHoaDonIdTC(hoaDonId),
          getDichVuSuDungTC(hoaDonId),
          getPhuThuByHoaDonIdTC(hoaDonId),
          getListVatTuHongThieuTC(hoaDonId),
        ]);
  ...
  const handleViewDetail = (idLoaiPhong) => {
    navigate(`/lookup/detail-ttdp/${idDatPhong}/${idLoaiPhong}`);
  };
