...
public class KhachHangLoginController {
    ...
    @PostMapping("/sign-in")
    public ResponseEntity<Optional<KhachHang>> login(@RequestBody KhachHangRegister request) {
        ...
        if (isAuthenticated) {
            ...
            return ResponseEntity.ok(khachHangService.KhachHangLogin(request.getEmail()));
        } else {
